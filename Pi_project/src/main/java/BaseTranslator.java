...
public class BaseTranslator {
	...
	public static int[] convertBase(int[] digits, int baseA, int baseB,
		...
		int[] digitstoConvert = digits.clone();
		int[] result = new int[precisionB];
		int carry;
		int x;
		for (int i = 0; i < precisionB; i++) {
			carry = 0;
			for (int j = digitstoConvert.length - 1; j >= 0; j--) {
				if (digitstoConvert[j] >= baseA || digitstoConvert[j] < 0) {
					return null;
				}
				x = (digitstoConvert[j] * baseB) + carry;
				digitstoConvert[j] = x % baseA;
				carry = x / baseA;
			}
			result[i] = carry;
		}
