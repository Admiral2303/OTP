...
public class AutoCompleteDictionaryTrie implements Dictionary, AutoComplete {
    ...
    public boolean addWord(String word) {
        ...
        for (int i = 0; i < addWord.length(); i++) {
            if (node.getValidNextCharacters().contains(addWord.charAt(i))) {
                node = node.getChild(addWord.charAt(i));
            } else {
                node = node.insert(addWord.charAt(i));
            }
        }
        if (!node.endsWord()) {
            size++;
    ...
    public boolean isWord(String s) {
        ...
        for (int i = 0; i < sToCheck.length(); i++) {
            if (node.getValidNextCharacters().contains(sToCheck.charAt(i))) {
                node = node.getChild(sToCheck.charAt(i));
            } else {
                return false;
            }
        }
     ...
     * @paramtext The text to use at the word stem
     * @paramn The maximum number of predictions desired.
     */
    @Override
